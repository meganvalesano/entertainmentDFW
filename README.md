...
Any changes you make here will be reflected in your group project URL: https://721-g4-26.netlify.app/

### 🔐 Local secrets via `secrets.toml`
- Copy `secrets.toml.example` to `secrets.toml` (this file is gitignored).
- Fill in your keys:
  - `openai.api_key`
  - `serpapi.api_key`
  - `plaid.client_id`, `plaid.secret`, `plaid.env` (e.g., `sandbox`)
  - Optional: `langsmith.project`
- Run `netlify dev` normally — the functions auto-load `secrets.toml`, so you no longer need to export env vars in the shell.
...
async function fetchReply(){
  const url = 'https://janetan--721-g4-26.netlify.app/.netlify/functions/fetchAI';

async function fetchCompetitors(productName) {
  const url = 'https:/janetan--721-g4-26.netlify.app/.netlify/functions/fetchCompetitors';
```
...
When users click the corresponding buttons in the landing page (https://721-g4-26.netlify.app/), the event listener in <code>index.js</code> for the corresponding button will be triggered to call either <code>fetchAI.js</code> for the advertising copy or <code>fetchCompetitors.js</code> to retrieve the competitors. 
...
You can visit your branch through https://janetan--721-g4-26.netlify.app/. If this version is approved by your team, you can further merge it to the main and visit it through https://721-g4-26.netlify.app/.
